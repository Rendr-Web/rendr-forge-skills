...
The exhaustive, stack-agnostic checks behind each audit category in `SKILL.md`. Work the categories in the order given there (money-and-identity-first). For *how* each check maps to a real codebase, pair this with the project's `STACK.md` (generated by `/setup-deslop`); the derivation pattern is in [../../setup-deslop/references/MAPPING-PATTERN.md](../../setup-deslop/references/MAPPING-PATTERN.md).

Each check is phrased as a **Hole to disprove**, not a box to tick. The job is to produce evidence the Hole is real: a runnable exploit test that makes the bad thing happen (preferred), or a precise code-inspection finding when a runnable test is genuinely infeasible from the audit context (with the required `Why no runnable test` justification — see [FINDING-FORMAT.md](./FINDING-FORMAT.md)). If you tried the test and the bad thing won't happen, the check passes; if you couldn't build the test at all, fall back to code-inspection rather than skipping the check.
